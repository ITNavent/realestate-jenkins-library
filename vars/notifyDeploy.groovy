def call(Map config) {
	...
	withCredentials([string(credentialsId: 'newrelic_api_key', variable: 'NR_API_KEY')]) {
		wrap([$class: 'BuildUser']) {
			def slackMessage = config?.slackMessage ?: "Deploy iniciado por ${BUILD_USER_ID} para ${newrelicAppName} finalizado el ${BUILD_TIMESTAMP} revision: ${gitRevision} branch: ${gitBranch}"
